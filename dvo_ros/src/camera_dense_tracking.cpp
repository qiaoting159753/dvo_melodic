...
CameraDenseTracker::CameraDenseTracker(ros::NodeHandle& nh, ros::NodeHandle& nh_private):CameraBase(nh, nh_private),
...
{
  std::cout << "Handle Config!" << std::endl;
  ...
  //   if(true)
  //   {
  //     vis_ = new dvo::visualization::PclCameraTrajectoryVisualizer(true);
  //     //vis_ = new dvo_ros::visualization::RosCameraTrajectoryVisualizer(nh_);
  //   }
  //   else
  //   {
  //     //Went to this path???????
  //     std::cout << "Create the visualizer 2" << std::endl;
  //     vis_ = new dvo::visualization::NoopCameraTrajectoryVisualizer();
  //   }
  // }
}
...
)
{
  ...
  if(!reference)
  {
    ...
    //vis_->camera("first")->color(dvo::visualization::Color::blue()).update(current->level(0), tracker->intrinsics(0), accumulated_transform).show();
  ...
  if(success)
  {
    ...
    //vis_->camera("current")->color(dvo::visualization::Color::red()).update(current->level(0), tracker->intrinsics(0), accumulated_transform).show();
  }
  else
  {
...
{
  Eigen::Matrix3d m = transform.rotation();
  Eigen::Vector3d v = transform.translation();
