...
bool BenchmarkNode::configure()
{
  ...
  std::cout << cfg_.CameraFile << std:: endl;
...
void BenchmarkNode::run()
{
  ...
  if(cfg_.VisualizationRequired())
  {
    ...
    if(cfg_.RenderVideo)
    {
      ...
      ((dvo::visualization::PclCameraTrajectoryVisualizer*) visualizer)->visualizer().setSize(1280, 960);
    }
  }
  else
  {
  ...
  for(std::vector<dvo_benchmark::RgbdPair>::iterator it = pairs.begin(); ros::ok() && it != pairs.end(); ++it)
  {
    ...
    if(cfg_.ShowGroundtruth)
    {
      ...
      visualizer->trajectory("groundtruth")->color(dvo::visualization::Color::green()).add(groundtruth_pose);
      
      visualizer->camera("groundtruth")->color(dvo::visualization::Color::green()).update(current->level(0), intrinsics, groundtruth_pose).show(cfg_.ShowEstimate ? dvo::visualization::CameraVisualizer::ShowCamera : dvo::visualization::CameraVisualizer::ShowCameraAndCloud);
